...
:gatherSettings
echo Gathering launch settings...
set TEMP_FILE="%APPDATA%\tempRMS6L"
"%JAVA%" -cp "%RAPIDMINER_HOME%"\lib\* com.rapidminer.launcher.JVMOptionBuilder > %TEMP_FILE%
...
:launch
echo Launching RapidMiner Studio BATCH now...
"%JAVA%" %JVM_OPTIONS% -cp "%RAPIDMINER_HOME%"\lib\*;"%RAPIDMINER_HOME%"\lib\jdbc\* com.rapidminer.launcher.CommandLineLauncher %CMD_LINE_ARGS%
goto startEnd
...
echo not change the rapidminer-batch.bat file.
echo.
pause
goto end
